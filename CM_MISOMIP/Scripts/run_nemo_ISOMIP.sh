...
INPUTDTA="/scratch/shared/egige60/input/nemo_${CONFIG}/DTA_${FORCING_EXP_ID}"  #- dta directory (can be $INPUTDIR)
...
if [ -z "${RST_START}" ] || [ -z "${RST_FILE}" ]; then
  echo '~!@#%^&* ERROR in run_nemo_ISOMIP.sh : needs two non-empty attributes'
  echo '                                       >>>>>>>>>>>>> stop !!'
  exit
fi
...
##- Specific treatment for ISOMIP restart/initial state:
...
##- Check frequency in dta_temp and dts_sal files and adapt namelist accordingly :
NTIME=`ncdump -h ${INPUTDTA}/dta_temp_y${YEAR}_${CONFIG}_${FORCING_EXP_ID}.nc |grep UNLIMITED | awk '{print $6}' |sed -e "s/(//g"`
if [ $NTIME -eq 365 ]; then
  DTA_FREQ="24"
elif [ $NTIME -eq 1 ]; then
  DTA_FREQ="-12"
else
  echo "~!@#%^& PROBLEM WITH time VARIABLE IN ${INPUTDTA}/dta_temp_y${YEAR}_${CONFIG}_${FORCING_EXP_ID}.nc"
  echo "        CHECK CONSISTENCY WITH namelist >>>>>>>> STOP"
  exit
fi

##-
sed -e "s/CCCC/${CONFIG}/g ; s/OOOO/${CASE}/g ; s/IIII/${YEAR0}0101/g ; s/NIT000/${NIT000}/g ; s/NITEND/${NITEND}/g ; s/DTAFREQ/${DTA_FREQ}/g" namelist_ref > tmp
mv -f tmp namelist_ref
...
if [ $YEARm1 -ge 0 ]; then
  ln -s -v -f ${INPUTDTA}/dta_temp_y${YEARm1}_${CONFIG}_${FORCING_EXP_ID}.nc dta_temp_y${YEARm1}.nc
  ln -s -v -f ${INPUTDTA}/dta_sal_y${YEARm1}_${CONFIG}_${FORCING_EXP_ID}.nc  dta_sal_y${YEARm1}.nc
fi
ln -s -v -f ${INPUTDTA}/dta_temp_y${YEAR}_${CONFIG}_${FORCING_EXP_ID}.nc   dta_temp_y${YEAR}.nc
ln -s -v -f ${INPUTDTA}/dta_sal_y${YEAR}_${CONFIG}_${FORCING_EXP_ID}.nc    dta_sal_y${YEAR}.nc
ln -s -v -f ${INPUTDTA}/dta_temp_y${YEARp1}_${CONFIG}_${FORCING_EXP_ID}.nc dta_temp_y${YEARp1}.nc
ln -s -v -f ${INPUTDTA}/dta_sal_y${YEARp1}_${CONFIG}_${FORCING_EXP_ID}.nc  dta_sal_y${YEARp1}.nc
...
do
  ...
  if [ $YEARm1 -ge 0 ]; then
    ln -s -v -f ${INPUTDTA}/${iZOOM}_dta_temp_y${YEARm1}_${CONFIG}_${FORCING_EXP_ID}.nc ${iZOOM}_dta_temp_y${YEARm1}.nc
    ln -s -v -f ${INPUTDTA}/${iZOOM}_dta_sal_y${YEARm1}_${CONFIG}_${FORCING_EXP_ID}.nc  ${iZOOM}_dta_sal_y${YEARm1}.nc
  fi
  ln -s -v -f ${INPUTDTA}/${iZOOM}_dta_temp_y${YEAR}_${CONFIG}_${FORCING_EXP_ID}.nc   ${iZOOM}_dta_temp_y${YEAR}.nc
  ln -s -v -f ${INPUTDTA}/${iZOOM}_dta_sal_y${YEAR}_${CONFIG}_${FORCING_EXP_ID}.nc    ${iZOOM}_dta_sal_y${YEAR}.nc
  ln -s -v -f ${INPUTDTA}/${iZOOM}_dta_temp_y${YEARp1}_${CONFIG}_${FORCING_EXP_ID}.nc ${iZOOM}_dta_temp_y${YEARp1}.nc
  ln -s -v -f ${INPUTDTA}/${iZOOM}_dta_sal_y${YEARp1}_${CONFIG}_${FORCING_EXP_ID}.nc  ${iZOOM}_dta_sal_y${YEARp1}.nc
  RSTN=`grep "from a restart file" namelist_ref | awk '{print $3}' | sed -e "s/\.//g"`
  NIT_RST=${NITENDM1}
  if [ $RSTN == "true" ]; then
    if [ $NIT_RST -eq 0 ]; then
    ...
    #  cp -p ${INPUTDTA}/${iZOOM}_dta_temp_${CONFPAR}_y${Y0}m${M0}.nc .
    #  cp -p ${INPUTDTA}/${iZOOM}_dta_sal_${CONFPAR}_y${Y0}m${M0}.nc .
    #fi
  fi
...
#- Check for NaNs and relaunch if some are found:
...
if [ $ISNAN -eq 0 ]; then
  ...
  # monitor nb of previous re-submission because of NaNs and allow no more than 3 to avoid endless loops :
  if [ -f check_NaN.db ]; then
    PREVNAN=`cat check_NaN.db`
  else
    PREVNAN=0
  fi
  PREVNAN=`expr $PREVNAN + 1`
  echo "$PREVNAN" > check_NaN.db
  if [ $PREVNAN -le 3 ]; then
    sbatch ./run_nemo_ISOMIP.sh ${RST_START} ${RST_FILE}
  else
    echo "   >> ALREADY 3 ATTEMPTS => STOPPING FOR GOOD"
  fi
  stop
fi
echo " "
